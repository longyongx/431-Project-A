...
chr_2020_raw <- read_csv(data_url, skip = 1, guess_max = 3200)
```
...
- The slope (0.08) of `some_college` is the predicted change in `life_expectancy` associated with a 1% increase in the number of adults with some form of post-secondary education. There is 90% confidence that the true slope is between 0.06 to 0.09.
...
ggplot(data = life_income, mapping = aes(x = income_cat, y = life_expectancy)) +
  ...
  labs(title="Life Expectancy is Higher for Higher Income Categories", y="Life Expectancy in Years",
       x="Income Category")
```
...
In the above plot, a negative association is observed between `uninsured_adults` and `life_expectancy` across all 330 counties. The majority of the data points appear to cluster around 5 to 10% uninsured adults. There are two very distinguishable outliers in the data set, at around 25% uninsured adults.
...
The residuals versus fitted values plot shows two clusters of fitted values: one around 77 years of age and one around 78 years of age. The majority of actual values have residual absolute values around or less than 3. Several points have residual absolute values at around 4.5 or higher.
